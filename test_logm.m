    ...
    for k = ids_min : ids_max
        ...
        normA = norm(mp(A), 1);
         ...
         if (j == 1)
             old_d = mp.Digits();
             mp.Digits(16);
             condest1(k) = funm_condest1(mp(A), @logm_mp, @logm_mp);
             mp.Digits(old_d);
         end
...
for j = 1:P
    ...
    % save .png
    if png_out
        filename = sprintf('pngfigs/logm_mp_perfprof_%03d', p);
        saveas(gcf, filename, 'png');
    end

    % tikz
    if tikz_out
        filename = sprintf('figs/logm_mp_perfprof_%03d.tikz', p);
    ...
    % save .png
    if png_out
        filename = sprintf('pngfigs/logm_mp_accuracy_%03d', p);
        saveas(gcf, filename, 'png');
    end
    ...
    if tikz_out
        ...
        filename = sprintf('figs/logm_mp_accuracy_%03d.tikz', p);
        matlab2tikz(filename, 'showInfo', false,...
